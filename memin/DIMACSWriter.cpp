...
void buildCNF(Solver* solver, vector<pair<int, int> >& literalToStateClass, 
              unsigned int numClasses, vector<vector<int> >& machineNextState, 
              vector<bool>& incompMatrix, vector<int>& pairwiseIncStates, 
              int maxInput, const int& verbosity)
{
	S = solver;
	clauses.clear();
	curClause.clear();
