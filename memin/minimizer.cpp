...
#include "ext-folding/memin/minisat/core/Solver.h"
#include "ext-folding/memin/minisat/utils/System.h"

#include "ext-folding/memin/KISSParser.h"
#include "ext-folding/memin/IncSpecSeq.h"
#include "ext-folding/memin/DIMACSWriter.h"
#include "ext-folding/memin/MachineBuilder.h"
...
int MeMin_Command(Abc_Frame_t *pAbc, int argc, char **argv) {
	...
	if ((argc <= 2) || (argv[argc - 1] == NULL) || (argv[argc - 2] == NULL) || (argv[argc - 2][0] == '-')) {
		usage();
		return 1;
	} else {
		parseKISSFile(argv[argc-2], machine, resetState, numInputBits, numOutputBits, firstStateReset);
	...
	for (int nClasses=pairwiseIncStates.size(); nClasses>=0; nClasses++) {
		...
		Solver S;
		buildCNF(&S, literalToStateClass, nClasses, nextStatesMap, incompMatrix, pairwiseIncStates, inputIDToIncSpecSeq.size()-1, verbosity);
		...
		if (ret == l_True){
			...
			writeKISSFile(newMachineNextStatesMap, newMachineOutputsMap, newResetState, numInputBits, numOutputBits, inputIDToIncSpecSeq, outFileName);

			gettimeofday(&end, 0);
			if (verbosity>0) cout << "Writing to KISS file: "<< (end.tv_sec*1e6 + end.tv_usec) - (start.tv_sec*1e6 + start.tv_usec) << " usec" << endl;

			if (verbosity>0) cout << "Result written to " << outFileName << endl;
			return 0;
		}
	}
	return 0;
